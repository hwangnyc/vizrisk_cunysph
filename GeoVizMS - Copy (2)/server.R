...
shinyServer(
        ...
        output$stateview <- renderPlot({
              ...
              a <- ggplot(side_tab(), aes(x=AGEG, y=Prop, fill=IMPRACE)) + geom_bar(stat="Identity") + scale_fill_brewer(type="qual", palette=3)
              k <- a + ylab("Prevalence Proportion") + labs(fill="Racial/Ethnic \n Group") + 
                theme(axis.title.x = element_blank(),text=element_text(size=16, family="sans"))
              b <- ggplot(side_tab2(), aes(x=AGEG, y=Prop, fill=SEX)) + geom_bar(stat="identity") + 
                scale_fill_manual(values=c(564,506))
